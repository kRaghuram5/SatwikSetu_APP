...
**Note:** follow this step only if you delete migrations folder
```bash
docker exec -it gateway alembic init migrations
```
...
```bash
docker exec -it gateway alembic upgrade head
```
...
# 4 seed data.
```bash
docker-compose exec gateway python -m app.cli seed
```
---
